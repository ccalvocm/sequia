...
def DEVELOP_SRM(root, Basin, alpha, Tcrit, plots):
    ...
    if plots:
        ...
        # propiedades del texto PRELIMINAR
        ...
        # SWE sim
        swe_sim = np.sum(2*snowAcc*A, axis = 1) / Atot
        swe_sim = pd.DataFrame(swe_sim, index = dates, columns = ['SWE simulado']) # SWE en m

        # SWe observado
        swe_obs = pd.read_csv(r'C:\Users\ccalvo\OneDrive - ciren.cl\Ficha_16_Coquimbo\02_SIG\02_Aguas sup\04_Regional\cuencas_cabecera\Rio Mostazal En Cuestecita\SWE_bands_ERA5h.csv', index_col = 0, parse_dates = True)
