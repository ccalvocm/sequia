...
def baseflow(strflow,alpha,beta,bf):
    ...
    strflow = pd.DataFrame(strflow, index = pd.date_range('2000-01-01',
pd.to_datetime('2000-01-01')+datetime.timedelta(days=len(strflow)-1),
 freq = '1d') )
    ...
    idx = pd.date_range(strflow.index[0],
str(date_end.year)+'-'+str(date_end.month)+'-'+str(date_end.days_in_month), 
freq = '1d')
    strflow = strflow.reindex(idx, fill_value=fill_value[0])
    # calcular el baseflow medio mensual
    df_BaseFlow = pd.DataFrame(bf, index = pd.date_range('2000-01-01',
pd.to_datetime('2000-01-01')+datetime.timedelta(days=len(bf)-1), freq = '1d') )
    ...
    df_baseq = pd.DataFrame(baseq, index = pd.date_range('2000-01-01',idx[-1],
                                                          freq = '1d') )
    # resample del flujo base mensual a diario
    baseq_day = df_baseq.reindex(pd.date_range('2000-01-01',date_end,
                                               freq='1d'), method='nearest')
