...
riegoreq=pd.read_csv(r'G:\OneDrive - ciren.cl\2022_ANID_sequia\Proyecto\3_Objetivo3\Resultados\Chalinga\unmetD.csv',
                   index_col=0)
riegoreq.index=index()
entradas=suma([hfF,GWin,rf,retornoRio,riego])
remanentesRiego=suma([GWout,suma([riego.multiply(-1)])])
salidas=suma([AP,IND,qDesemb,GWout])
...
def residual(inefi=1):
    df=pd.DataFrame(index=index())
    df['headflows']=dfTocol(hfF)
    df['GWin']=dfTocol(GWin)+dfTocol(riego)
    df['qDesemb']=dfTocol(qDesemb.multiply(-1))
    df['IND']=dfTocol(IND.multiply(-1))
    df['AP']=dfTocol(AP.multiply(-1))
    df['PEC']=dfTocol(AP.multiply(-1))

    df['GWout']=inefi*dfTocol(GWout.multiply(-1))+dfTocol(riego.multiply(1))
    ...
    'Uso industrial','Agua potable','Uso pecuario','Salidas agua subterránea','Riego','Retornos de agua']
    df.columns=df.apply(lambda x: x.name + ' (Hm3/mes)')
...
r = opt.root(residual, x0=1.1288985823336968, method='hybr')
print(r.x)
